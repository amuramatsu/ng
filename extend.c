/* $Id: extend.c,v 1.7.2.13 2007/01/11 14:50:41 amura Exp $ */
/*
...
/* extern MAP_ELEMENT *ele;	must be set before calling */
{
	...
	if (ele > &curmap->map_element[0]
	    && (funct!=prefix || (ele-1)->k_prefmap==NULL)) {
	    n1 = c - (ele-1)->k_num;
	}
	else
	    n1 = HUGE;
...
/* split into two statements due to bug in OSK cpp */
	...
	if(fp != unbindtokey && fp != localunbind) {
	    ...
	    status = bindkey(&curmap, (char *)NULL, key.k_chars, key.k_count);
	break;
    case BINDNO:
#endif
	inmacro = TRUE;
