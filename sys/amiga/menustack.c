/* $Id: menustack.c,v 1.1 2000/06/27 01:48:01 amura Exp $ */
/*
 ...
 * Initial revision
 *
 * Revision 1.2  1999/05/21  00:57:06  amura
 * For KANgee 4.2.1
 *
 * Revision 1.1  1999/05/21  00:55:43  amura
 * Initial revision
 *
 */
...
/* For Font Sensitive by H.Konishi 1995/6/3 */
...
#define SCREENWIDTH	(WBInfo . Width)
                                /* For Font Sensitive by H.Konishi 1995.6.4 */
#define SCREENFONT      (WBInfo . Font->ta_YSize) 
...
/*      m->TopEdge = LastMenuItem->TopEdge + 10; */
                           /* For Font Sensitive by H.Konishi 1995.6.4 */
      m->TopEdge = LastMenuItem->TopEdge + SCREENFONT + 2; 
    ...
    *  
    Attempt 1:
      if ( GfxBase != NULL )
      m->Height = (GfxBase->DefaultFont->tf_YSize ) + 2; 
      else  
    */ 
   m->Height = SCREENFONT + 2;
...
/*    m->TopEdge = LastSubItem->TopEdge + 10; */
                       /* For Font Sensitive by H.Konishi 1995.6.4 */
      m->TopEdge = LastSubItem->TopEdge + SCREENFONT + 2; 
   if (flags & CHECKIT)
      m->Width += CHECKWIDTH;
   if (flags & COMMSEQ)
      m->Width += COMMWIDTH + 20;
