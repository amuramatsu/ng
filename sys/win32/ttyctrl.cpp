/* $Id: ttyctrl.cpp,v 1.10 2001/11/23 11:56:56 amura Exp $ */
...
/*
 * $Log: ttyctrl.cpp,v $
 * Revision 1.10  2001/11/23 11:56:56  amura
 * Rewrite all sources
 *
...
#define KDrawText(hdc, str, len, rect, f) do {				\
    HFONT hOldFont;							\
...
#ifdef	DROPFILES	/* 00.07.07  by sahf */
    int   GetDropFiles(HLOCAL *);
#endif	/* DROPFILES */
    void  SetupEvent(HANDLE);
    void  ClearScreen();
    void   PutChar(BYTE);
    void  PutKChar(BYTE, BYTE);
    void  GotoXY(WORD, WORD);
    void  EraseEOL();
    void  EraseEOP();
    void  PutLine(WORD, WORD, LPCSTR);
    void  Flush();
    BOOL  Kbhit() const;
    int	  GetChar();
    DWORD GetWH() const;
    void  AddMetaChar(TCHAR);
    void  AddChar(TCHAR);
    void  Command(WPARAM, LPARAM);
    void  SetKeyboardLocale(DWORD);
...
public:
    ...
    friend LRESULT CALLBACK TtyViewWndProc(HWND hWnd, UINT msg,
...
TtyView::SetFontValues(LOGFONT *lf, HDC hdc)
{
	...
	if (point > 0) {
	    linespace = RegQueryDWord(HKEY_CURRENT_USER,
				      NGREGKEY, NGLINESPACEVAL);
#ifndef KANJI
    calcsizes:
#endif
...
#endif
	    if (pt.y < 0)		/* obtain an absolute value */
		pt.y = -pt.y;
...
TtyViewWndProc(HWND hWnd,  UINT msg, WPARAM wParam, LPARAM lParam)
{
...
#if 0
    case TTYM_SETTAB:
	pWindow->SetTab((DWORD) wParam, (BOOL) lParam);
	break;
    case TTYM_SETHMARGIN:
	pWindow->SetHMargin((DWORD) wParam, (BOOL) lParam);
	break;
#endif
    case TTYM_SETEVENT:
	pWindow->SetupEvent((HANDLE) wParam);
	break;
#ifdef CONFIG_FONT
...
#endif
    case TTYM_GOTOXY:
	pWindow->GotoXY(HIWORD(wParam), LOWORD(wParam));
	break;
    case TTYM_PUTCHAR:
	pWindow->PutChar((BYTE) wParam);
	break;
    case TTYM_PUTKCHAR:
	pWindow->PutKChar((BYTE) HIWORD(wParam), (BYTE) LOWORD(wParam));
	break;
    case TTYM_PUTLINE:	
	pWindow->PutLine(HIWORD(wParam), LOWORD(wParam), (LPCSTR) lParam);
	break;
    case TTYM_ERASEEOL:
	pWindow->EraseEOL();
	break;
	...
	break;
    case TTYM_KBHIT:
	return pWindow->Kbhit();
    case TTYM_GETCHAR:
	return pWindow->GetChar();
    case TTYM_GETWINDOWEVENT:
	return pWindow->GetWindowEvent();
#ifdef	DROPFILES	/* 00.07.07  by sahf */
    case TTYM_DROPFILES:
	return  pWindow->GetDropFiles((LPHANDLE) lParam);
#endif	/* DROPFILES */
    case TTYM_GETWH:
	return pWindow->GetWH();
    case TTYM_COMMAND:
